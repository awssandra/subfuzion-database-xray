...
class Database {
  ...
  async updateVote(vote) {
    ...
    var seg = xray.GetSegment().addNewSubsegment("updateVote");
    ...
    if (!result.ok) {
      let error = new Error(JSON.stringify(result.lastErrorObject));
      seg.close(error)
      throw error;
    }

    seg.addMetadata("voter_id", vote.voter_id, "votes");
    seg.addMetadata("result", result.value, "votes");
